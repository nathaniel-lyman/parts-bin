...
describe('pagination slice', () => {
  it('sets pageIndex and preserves pageSize', () => {
    const next = gridReducer(DEFAULT_STATE, { type: 'setPageIndex', pageIndex: 3 })
  ...
  it('sets pageSize and resets pageIndex', () => {
    const paged = gridReducer(DEFAULT_STATE, { type: 'setPageIndex', pageIndex: 4 })
    const next = gridReducer(paged, { type: 'setPageSize', pageSize: 50 })
