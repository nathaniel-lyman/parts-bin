...
export function Button({ variant = 'secondary', size = 'default', loading = false, disabled, className, children, ...rest }: ButtonProps) {
  return (
    <button
      className={cx(
        'inline-flex items-center justify-center gap-1.5 whitespace-nowrap rounded-sm text-[14px] font-medium transition-[background,opacity] duration-[var(--motion-moderate-01)] ease-[var(--ease-productive)] disabled:opacity-50',
        variants[variant],
        sizes[size],
        className,
      )}
