...
test.describe('command shortcuts', () => {
  test('run workspace, grid, and assistant commands without opening the palette', async ({ page }) => {
    ...
    await expect(page.getByText('Saved view Review focus')).toBeVisible()
  ...
  test('ignore text-entry focus but still work from checkbox focus', async ({ page }) => {
    ...
    await page.keyboard.press('r')
    await expect(page.getByText(/Review focus/)).toHaveCount(0)
  ...
  test('movement shortcut opens evidence-backed assistant answer', async ({ page }) => {
    ...
    await expect(page.getByRole('dialog', { name: 'Assistant' })).toBeVisible()
    await expect(page.getByText(/Signed movement is net positive/)).toBeVisible({ timeout: 10000 })
    await expect(page.getByText('Evidence used')).toBeVisible()
    await expect(page.getByText(/Chart: Signed movement \(\$k\), 10 monthly rows/)).toBeVisible()
    await expect(page.getByText(/Separation: chart evidence uses monthly movement rows/)).toBeVisible()
    await expect(page.getByRole('dialog', { name: 'Command palette' })).toHaveCount(0)
  })
})
